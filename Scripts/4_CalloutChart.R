...
if (length(reportYear) == 1 & columnTest) {
  ...
  p <- 
    ...
    guides(fill=FALSE) +
    theme_few() +
    theme(plot.title = element_text(hjust = 0.5, size = 25), plot.subtitle = element_text(hjust = 0.5, size = 20), plot.caption = element_text(size = 15), axis.text.y=element_text(size = 15))
  p
